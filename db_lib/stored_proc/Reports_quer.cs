...
namespace db_lib.stored_proc
{
    ...
    {
        // get all transactions for the reports form
        public string Quer_GetAllTrans { get; } = @"With main as (SELECT t.emp_no , e.name, e.job, e.dept, e.section, c.name AS item, sc.name AS subcat,  t.unit_price, t.quantity, t.delivery_date, t.remarks , u.user_name, t.id FROM transactions t Left JOIN items c ON t.item_id = c.id Left JOIN employees e ON e.code = t.emp_no Left Join subcats sc ON sc.id = t.subcat_id Left JOIN users u ON u.id = t.user_id) Select * from main";
        // get all transactions based on specific month and year as well as an additional search word in the columns of employee#, department, item, and department section
        ...
        // same as the previous but filters with month only thourghout all years
        ...
        // same as previous but filters by one year 
        ...
        // filter by search keword regardless of the month and year
